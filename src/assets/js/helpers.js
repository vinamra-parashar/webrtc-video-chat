export default {
  ...
  toggleModal(id, show, text) {
    let el = document.getElementById(id);
    let span = document.getElementById("call-user");
  ...
  setLocalStream(stream, mirrorMode = true) {
    ...
    if (
      sessionStorage.getItem("video") === "false" ||
      sessionStorage.getItem("mic") === "false"
    ) {
      localVidElem.srcObject = stream;

      if (sessionStorage.getItem("video") === "false") {
        localVidElem.srcObject.getVideoTracks()[0].enabled = false;
        let vicon = document.querySelector("#video-icon");
        vicon.classList.remove("fa-video");
        vicon.classList.add("fa-video-slash");
        document
          .getElementById("toggle-video")
          .setAttribute("title", "Show Video");
      }
      if (sessionStorage.getItem("mic") === "false") {
        localVidElem.srcObject.getAudioTracks()[0].enabled = false;
        let micon = document.querySelector("#mic-icon");
  ...
  adjustVideoElemSize() {
    ...
    count.textContent = totalRemoteVideosDesktop + 1;
    // console.log('totalRemoteVideosDesktop', totalRemoteVideosDesktop);
    let newWidth =
